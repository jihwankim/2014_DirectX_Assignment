...
HRESULT InitGeometry()
{
	...
	if (FAILED(D3DXLoadMeshFromX(L"Tiger0.x", D3DXMESH_SYSTEMMEM,
		...
		&g_pMesh[0])))
	{
		MessageBox(NULL, L"Could not find tiger0.x", L"Meshes.exe", MB_OK);
		return E_FAIL;
	}
	// Load the mesh from the specified file
	if (FAILED(D3DXLoadMeshFromX(L"Tiger1.x", D3DXMESH_SYSTEMMEM,
		...
		&g_pMesh[1])))
	{
		MessageBox(NULL, L"Could not find tiger1.x", L"Meshes.exe", MB_OK);
		return E_FAIL;
	}
...
//-----------------------------------------------------------------------------
VOID SetupMatrices(int ObjectNum)
{
    // Set up world matrix
